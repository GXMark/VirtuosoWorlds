...
void URegionServerBridge::EnsureSpatialState(APlayerController* PC)
{
	...
	for (const FVMActor& A : ActorItems)
	{
		...
		{
			AddMeshSpatialItem(A, State);
		}
		else if (A.type == FVActorType::DecalActor)
		{
			AddDecalSpatialItem(A, State);
		}
		else if (A.type == FVActorType::PointLightActor)
		{
			AddPointLightSpatialItem(A, State);
		}
		else if (A.type == FVActorType::SpotLightActor)
		{
			AddSpotLightSpatialItem(A, State);
		}
	}
}

FVMTransformNet URegionServerBridge::MakeNetTransform(const FVMTransform& In) const
{
	FVMTransformNet Out;
...
FVMMeshComponentNet URegionServerBridge::MakeMeshNet(const FVMMeshComponent& In) const
{
...
FVMPointLightComponentNet URegionServerBridge::MakePointLightNet(const FVMPointLightComponent& In) const
{
...
FVMSpotLightComponentNet URegionServerBridge::MakeSpotLightNet(const FVMSpotLightComponent& In) const
{
...
FVMDecalComponentNet URegionServerBridge::MakeDecalNet(const FVMDecalComponent& In) const
{
